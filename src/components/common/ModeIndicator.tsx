...
export function ModeIndicator({ className = "" }: ModeIndicatorProps) {
    ...
    const clearMode = () => {
        ...
        try {
            localStorage.removeItem("ss_api_mode");
        } catch {}
    ...
    return (
        ...
        >
            <div
                className={`${bgColor} ${textColor} px-2 py-0.5 sm:px-3 sm:py-1 rounded-full shadow flex items-center justify-center gap-1 sm:gap-2`}
            >
                <Icon className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                <span className="text-xs font-semibold text-center hidden sm:inline">
                    {label}
                </span>
                <span className="text-xs font-semibold text-center sm:hidden">
                    {label.split(" ")[0]}
                </span>
                <button
                    onClick={clearMode}
