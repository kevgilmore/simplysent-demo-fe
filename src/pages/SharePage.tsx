...
export function SharePage() {
    ...
    const [visitPingInterval, setVisitPingInterval] =
    ...
    const sendInteraction = async (
        ...
        try {
            ...
            if (!response.ok) {
                console.warn(
                    `Interaction ${event} failed with status ${response.status}`,
                );
    ...
    const sendVisitPing = async () => {
        try {
            ...
            if (!response.ok) {
                console.warn(
                    `Visit ping failed with status ${response.status}`,
                );
    ...
    const trackSharedVisit = async () => {
        try {
            ...
            if (!response.ok) {
                console.warn(
                    `Tracking event visit_shared_link failed with status ${response.status}`,
                );
                // Show toast for tracking errors in sandbox modes
                if (isAnySandboxMode()) {
                    showApiError(
            ...
            const fetchFromAPI = async () => {
                try {
                        ...
                        const transformedContext = {
                            ...
                            relationship: data.context.relationship
                                ? data.context.relationship
                                      .charAt(0)
                                      .toUpperCase() +
                                  data.context.relationship.slice(1)
                                : "",
                            occasion: data.context.occasion
                                ? data.context.occasion
                                      .charAt(0)
                                      .toUpperCase() +
                                  data.context.occasion.slice(1)
                                : "",
                            sentiment: data.context.sentiment
                                ? data.context.sentiment
                                      .charAt(0)
                                      .toUpperCase() +
                                  data.context.sentiment.slice(1)
                                : "",
                            interests: data.context.interests || [],
                            favoritedrink: data.context.favourite_drink
                                ? data.context.favourite_drink
                                      .charAt(0)
                                      .toUpperCase() +
                                  data.context.favourite_drink.slice(1)
                                : "",
                    ...
                    if (
                        ...
                        console.error(
                            "No recommendations found in API response",
                        );
                        setError(true);
                    }
                    setLoading(false);
                    ...
                    console.error(
                        "Failed to fetch recommendation from API:",
                        err,
                    );
            ...
            >
                <div className="text-center space-y-6">
                    ...
                    <p className="text-gray-600 max-w-md mx-auto">
                        This recommendation link may be invalid, expired, or the
                        recommendation was created on a different device. Please
                ...
                >
                    <div className="relative bg-gradient-to-r from-purple-50 to-indigo-50 rounded-2xl p-6 border border-purple-100">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                                ...
                                {contextInfo.clothesSize && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-gray-700">
                                            <strong>Clothes Size:</strong>{" "}
                                            {contextInfo.clothesSize}
                                        </span>
                                    </div>
                                )}
                                {(contextInfo.minBudget ||
                                    contextInfo.maxBudget) && (
                                    <div className="flex items-center gap-2">
                                        <span className="text-purple-600">
                                            💰
                                        </span>
                        ...
                        {contextInfo.interests &&
                            contextInfo.interests.length > 0 && (
                            ...
                            <div className="p-8">
                                ...
                                <div className="flex items-center justify-between mb-6">
                                    <div className="flex gap-2">
                                        <motion.button
                                            onClick={() =>
                                                handleRating(products[0].sku, 1)
                                            }
                                            whileHover={{ scale: 1.1, y: -2 }}
                                            whileTap={{ scale: 0.9 }}
                                            animate={
                                                productRatings[products[0].sku]
                                                    ? {}
                                                    : { y: [0, -8, 0] }
                                            }
                                            transition={
                        ...
                        <div className="relative overflow-hidden bg-gradient-to-r from-emerald-500 to-teal-500 rounded-xl shadow-lg">
                            ...
                            <div className="relative z-10 px-6 py-4 text-center">
                                ...
                                <p className="text-white/90 text-sm">
                                    Use the thumbs up/down buttons to show which
                                    gifts you think they'd love most
                                </p>
                            </div>
                        </div>
                    </motion.div>
