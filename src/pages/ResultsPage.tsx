...
export function ResultsPage() {
    ...
    const getFeedbackCount = (): number => {
        return Object.values(productFeedback).filter(
        ...
        try {
            ...
            if (response.ok) {
                ...
                try {
                    ...
                    if (response.ok) {
                        console.log(
                            "✅ Survey completion tracked successfully",
                        );
                    } else {
                        console.warn(
                            "⚠️ Failed to track survey completion:",
                            response.status,
                        );
                    }
                } catch (error) {
                    console.error(
                        "❌ Error tracking survey completion:",
                        error,
                    );
                }
            } else {
    ...
    useEffect(() => {
        const handleMessage = (event: MessageEvent) => {
            ...
            if (event.data && typeof event.data === "object") {
                ...
                const eventType =
                    event.data.type || event.data.eventType || event.data.event;
                ...
                if (
                    ...
                    console.log(
                        "Form submitted/completed! Event type:",
                        eventType,
                    );
    ...
    useEffect(() => {
        if (recommendationId && formData) {
            console.log(
                "🎯 Sending visit_start from ResultsPage with rec_id:",
                recommendationId,
            );
            const urlParams = new URLSearchParams(window.location.search);
    ...
    useEffect(() => {
        if (recommendationId && recommendations && recommendations.length > 0) {
            ...
            console.log(
                "Stored recommendation data in localStorage for sharing:",
                recommendationId,
            );
        }
    ...
    useEffect(() => {
        if (recommendationId) {
            ...
            if (!genieTrainingState) {
                try {
                    ...
                    if (storedData) {
                        ...
                        console.log(
                            "🔄 Loaded genie training state from localStorage:",
                            genieTrainingState,
                        );
                    }
                } catch (error) {
                    console.error(
                        "Error loading genie training state from localStorage:",
                        error,
                    );
                }
            }
            ...
            if (genieTrainingState) {
                console.log(
                    "🔄 Restoring genie training state:",
                    genieTrainingState,
                );
                ...
                if (genieTrainingState.recommendationRating > 0) {
                    setRecommendationRating(
                        genieTrainingState.recommendationRating,
                    );
    ...
    useEffect(() => {
        if (isAnySandboxMode()) {
            const addLabelsAndRating = localStorage.getItem(
                "add_labels_and_rating",
            );
    ...
    useEffect(() => {
        if (isAnySandboxMode() && products.length > 0) {
            ...
            if (shouldAutoFill) {
                console.log(
                    "🧪 Auto-filling feedback in sandbox mode with",
                    products.length,
                    "products",
                );
    ...
    const otherRecommendations =
        products && products.length > 1 ? products.slice(1) : [];
    ...
    useEffect(() => {
        if (products.length > 0 && recommendationId && formData) {
            try {
                ...
                localStorage.setItem(
                    `recommendation_${recommendationId}`,
                    JSON.stringify(fullRecommendationData),
                );
                console.log(
                    "💾 Stored full recommendation data in localStorage:",
                    recommendationId,
                );
            } catch (error) {
    ...
    const handleFeedback = async (sku: string, isGood: boolean) => {
        ...
        try {
            ...
            if (!response.ok) {
                console.warn(
                    `Feedback interaction failed with status ${response.status}`,
                );
    ...
    const buildInteractionHeaders = () => {
        ...
        const headers: Record<string, string> = {
            "Content-Type": "application/json",
            "x-anon-id": anonId,
            "x-request-id": "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(
                /[xy]/g,
                function (c) {
                    const r = (Math.random() * 16) | 0;
                    const v = c === "x" ? r : (r & 0x3) | 0x8;
    ...
    const getModalHeight = () => {
        if (!isSubmitted && !hasSubmittedComment) {
            ...
            (isSubmitted || hasSubmittedComment) &&
            showEmailFallback &&
            !showEmailInput &&
            !hasSubmittedEmail
        ) {
    ...
    const handleCopyLink = async () => {
        const baseUrl = isAnySandboxMode()
            ? "http://localhost:3000"
            : "https://simplysent.co";
        ...
        if (modalFeedback.trim()) {
            try {
                ...
                if (!commentResponse.ok) {
                    ...
                    setModalError(
                        `Failed to submit comment: ${commentResponse.status}`,
                    );
                    return;
                }
    ...
    const handleEmailNotify = async () => {
        console.log("handleEmailNotify called", {
            recommendationId,
            emailAddress,
            isLoading,
        });
        ...
        try {
            ...
            if (!emailResponse.ok) {
                ...
                throw new Error(
                    `Email submission failed: ${emailResponse.status} - ${errorText || "Unknown error"}`,
                );
            }
            ...
            if (error instanceof Error) {
                if (error.message.includes("Comment submission failed:")) {
                    setModalError(
                        `Comment error: ${error.message.split("Comment submission failed: ")[1]}`,
                    );
                } else if (error.message.includes("Email submission failed:")) {
                    setModalError(
                        `Email error: ${error.message.split("Email submission failed: ")[1]}`,
                    );
                            ...
                            <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-2xl shadow-lg p-6 border border-purple-100">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <p className="text-sm text-gray-600">
                                        ...
                                        so they can see your personalized gift
                                        recommendations!
                                    </p>
                                </div>
                            </div>
                ...
                >
                    <div className="text-center mb-6 px-4 sm:px-0 text-white">
                        ...
                        <p className="text-white/90">
                            And your budget of £{formData.minBudget} - £
                            {formData.maxBudget}
                        </p>
                    </div>
                    ...
                    >
                        <div className="relative">
                            <img
                                ...
                                alt={getProductTitle(topRecommendation)}
                                className="w-full h-64 object-contain bg-white p-4"
                                onError={(e) => {
                                    console.error("Image load error:", e);
                                    e.currentTarget.src =
                                        "https://cerescann.com/wp-content/uploads/2016/07/Product-PlaceHolder.jpg";
                                }}
                            />
                            ...
                            <div className="absolute top-4 left-4">
                                <div className="bg-white text-[#343e47] px-4 py-1 rounded-full shadow-lg border-2 border-[#343e47]">
                                    <div className="flex items-center">
                                        <span
                                            className="font-black text-base tracking-tight mr-0.5"
                                            style={{
                                                fontWeight: "900",
                                                textShadow:
                        ...
                        <div className="p-6">
                            <h3 className="text-2xl font-bold text-gray-900 mb-2 text-center">
                                {truncateText(
                                    getProductTitle(topRecommendation),
                                    60,
                                )}
                            </h3>
                            {/* Ratings removed */}
                            ...
                            <div className="space-y-3">
                                <a
                                    ...
                                    onClick={() =>
                                        handleLinkClick(topRecommendation.sku)
                                    }
                                ...
                                <div className="text-center py-2 px-4">
                                    <p className="text-sm text-gray-600 font-medium leading-relaxed">
                                        ...
                                        Add to your Amazon wishlist or cart
                                    </p>
                                </div>
                                ...
                                <div className="relative group">
                                    ...
                                    <div className="text-center mt-3">
                                        <p className="text-sm text-gray-600">
                                            {productFeedback[0] === null
                                                ? "✨ Teach your genie about this gift"
                                                : productFeedback[0] === "up"
                                                  ? '🧞‍♂️ Genie learned: "More like this!" 💚'
                    ...
                    <div className="text-center relative z-10">
                        <div className="flex items-center justify-center gap-4 mb-6">
                            {getFeedbackCount() === products.length ? (
                                ...
                                <div
                                    className={`${getFeedbackCount() === 0 ? "opacity-50" : getFeedbackCount() < products.length ? "animate-pulse" : "animate-bounce"}`}
                                >
                                    <img
                                        src="/genie3.png"
                                        alt="Gift Genie"
                            ...
                            <div>
                                ...
                                >
                                    {getFeedbackCount() === 0
                                        ? "Your Gift Genie Needs Training"
                                        : getFeedbackCount() < products.length
                                          ? "Training Your Gift Genie"
                                ...
                                >
                                    {getFeedbackCount() === 0
                                        ? "Your genie is weak - rate products to train it!"
                                        : getFeedbackCount() < products.length
                                          ? "Each rating makes your genie stronger!"
                                        ...
                                        >
                                            ...
                                            >
                                                <img
                                                    src={
                                                        i < getFeedbackCount()
                                                            ? "/magic_crystal_selected.png"
                                                            : "/magic_crystal.png"
                                                    }
                                                    alt={
                                                        i < getFeedbackCount()
                                                            ? "Selected Magic Crystal"
                                                            : "Magic Crystal"
                                                    }
                            ...
                            >
                                <div className="bg-gradient-to-r from-yellow-400 to-orange-500 p-4 flex items-center justify-between">
                                    <h3 className="text-white font-bold text-lg">
                                        5 Questions for £5 Amazon Voucher
                                    </h3>
                                    <button
                                        onClick={() =>
                                            setShowVoucherIncentive(false)
                            ...
                            {otherRecommendations.map((item, index) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className="bg-gradient-to-b from-purple-50 to-pink-50 rounded-xl shadow-md overflow-hidden border border-gray-100 flex flex-col"
                                    >
                                        ...
                                        <div className="relative">
                                            ...
                                            <div className="absolute top-4 left-4">
                                                <div className="bg-white text-[#343e47] px-3 py-0.5 rounded-full shadow-md border border-[#343e47]">
                                                    <div className="flex items-center">
                                                        <span
                                                            className="font-black text-sm tracking-tight mr-0.5"
                                                            style={{
                                                                fontWeight:
                                                                    "900",
                                                                textShadow:
                    ...
                    >
                        {getFeedbackCount() < products.length ? (
                            <>
                                ...
                                Rate all products to unlock better
                                recommendations
                            </>
                        ) : recommendationRating === 0 ? (
                            <>
                                <StarIcon className="w-5 h-5" />
                                Add a star rating to unlock better
                                recommendations
                            </>
                        ) : (
                            <>
                        ...
                        >
                            {isSubmitted || hasSubmittedComment ? (
                                ...
                                >
                                    ...
                                    >
                                        {titleIndex === 0 &&
                                            "🧞‍♂️ Cosmic gift energy detected"}
                                        {titleIndex === 1 &&
                                            "🧞‍♂️ Running a multi-dimensional vibe check"}
                                        {titleIndex === 2 &&
                                            "🧞‍♂️ These things can't be rushed."}
                                    </motion.h3>
                                ...
                                <div className="px-3 md:px-6 relative">
                                    ...
                                    <div className="flex justify-center">
                                        <button
                                            ...
                                            className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-semibold py-3 px-6 rounded-lg transition-all transform hover:scale-[1.02] shadow-lg hover:shadow-xl flex items-center justify-center gap-2"
                                        >
                                            <SparklesIcon className="w-4 h-4" />
